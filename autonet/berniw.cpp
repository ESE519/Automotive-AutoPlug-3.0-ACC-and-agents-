/***************************************************************************
    ...
    version              : $Id: berniw.cpp,v 1.38.2.1 2008/05/29 22:40:14 berniw Exp $
...
static void drive(int index, tCarElt* car, tSituation *situation)
{
	...
	if (currenttime != situation->currentTime) {
		...
		for (int i = 0; i < situation->_ncars; i++) {
			ocar[i].update();
			std::cout<<"Ocar.speed = "<<ocar[i].getSpeed()<<" Ocar.currentSegId = "<<ocar[i].getCurrentSegId()<<" Ocar.currentCursor = ["<<ocar[i].getCurrentPos()[0].x<<" "<<ocar[i].getCurrentPos()[0].y<<"]"<<std::endl;
		}
	}
